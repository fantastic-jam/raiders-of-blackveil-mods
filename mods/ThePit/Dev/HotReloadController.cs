...
namespace ThePit.Dev {
    internal static class HotReloadController {
        ...
        internal static void Initialize(Harmony harmony, string dllPath) {
            ...
            if (_hudUpdate == null) {
                ThePitMod.PublicLogger.LogWarning("[HotReload] BaseHUDPage.Update not found — F9 unavailable.");
            }
        }
        ...
        internal static void Enable() {
            ...
            ThePitMod.PublicLogger.LogInfo("[HotReload] Hooked BaseHUDPage.Update. Press F9 to reload.");
        }
        ...
        private static void HudUpdatePostfix() {
            ...
            ThePitMod.PublicLogger.LogInfo("[HotReload] F9 pressed.");
            if (string.IsNullOrEmpty(_dllPath) || !File.Exists(_dllPath)) {
                ThePitMod.PublicLogger.LogWarning($"[HotReload] DLL not found: {_dllPath}");
                return;
            }
            ThePitMod.PublicLogger.LogInfo("[HotReload] Reloading...");
            try {
                UnityHotReload.LoadNewAssemblyVersion(Assembly.GetExecutingAssembly(), _dllPath);
                ThePitMod.PublicLogger.LogInfo("[HotReload] Done.");
            }
            catch (Exception ex) {
                ThePitMod.PublicLogger.LogError($"[HotReload] Failed: {ex}");
            }
        }
    }
}
