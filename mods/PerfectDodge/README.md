...
This mod registers with [WMF](https://github.com/fantastic-jam/raiders-of-blackveil-mods/releases?q=WildguardModFramework) as a **Mod**. When WMF is installed, an **Allow Mods** toggle appears in the host setup screen. If the host sets it to **No**, PerfectDodge is disabled for that session and the session name will show a **(modded)** suffix when it's active.
