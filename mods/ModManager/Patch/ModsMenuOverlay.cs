...
namespace ModManager.Patch {
    internal sealed class ModsMenuOverlay {
        ...
        // Currently open mod settings (null = ModManager toggle list is shown)
        ...
        internal void Open() {
            foreach (var (guid, toggle) in _toggles) {
                toggle.Value = ModManagerConfig.IsEnabled(guid);
            }
        ...
        private void SelectModManagerEntry() {
        ...
        private VisualElement Build() {
            ...
            _modManagerBtn = new NavButton("Mods") { Active = true, OnClickedEvent = SelectModManagerEntry };
            _leftCursor.RegisterItem(_modManagerBtn);
            leftBar.Add(_modManagerBtn);

            // One entry per mod that has a custom settings menu
            _menuEntries.Clear();
            foreach (var mod in ModManagerRegistrants.AllMods().Where(m => m.MenuName != null)) {
        ...
        private VisualElement BuildToggleList() {
            ...
            var allMods = ModManagerRegistrants.AllMods().ToList();
            foreach (var mod in allMods) {
                ...
                if (mod.IsManaged && !_isInGameMenu) {
                    toggle.Value = ModManagerConfig.IsEnabled(guid);
                    toggle.OnValueChangedCallback = v => ModManagerConfig.GetEntry(guid).Value = v;
                    _toggles.Add((guid, toggle));
                    _rightCursor.RegisterItem(toggle);
                } else {
                    toggle.Value = mod.IsManaged ? ModManagerConfig.IsEnabled(guid) : true;
